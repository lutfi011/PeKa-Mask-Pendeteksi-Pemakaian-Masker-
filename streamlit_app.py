...
def detect_mask(image):
    ...
    for i in range(0, detection.shape[2]): # loop through the detection
        ...
        if confidence: # if the confidence is greater than the selected confidence from the side bar
...
def about():
    ...
    st.write(
            '''
		Aplikasi ini dibuat menggunakan algoritma mtcnn dan berguna untuk medeteksi pemakaian masker
...
def main():
    ...
    st.write("**Menggunakan Python, Tensorflow and OpenCV**") # streamlit function to display text
    


    activities = ['Home', 'About']
    choice = st.selectbox("MENU", activities) # streammlit function to display box selector
    # image, label = detect_mask(image=image)

    if choice == "Home": # If user chooses Home page
        st.write("") # display this
        image_file = st.file_uploader("Masukkan gambar", type=['jpeg', 'jpg', 'png']) # streamlit function to upload file
        

        if image_file is not None:  #confirm that the image is not a 0 byte file


            st.image(image_file, width=240) # then display a sidebar of the uploaded image
            
            if st.button("Deteksi"): # process button
